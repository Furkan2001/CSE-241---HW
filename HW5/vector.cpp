...
Tetris :: Tetris (int* coordinate)
{
    ...
    vector < vector < char > > temp(coordinate[0], vector<char>(coordinate[1]));
...
int Tetris :: numberOfMoves() const
{
...
/* I will return tetris_board vector if there is any tetromino in it in this function.. If there is no tetromino in tetris_board I will throw an exception...*/
MoveActions Tetris :: lastMove() const
{
    if (flag != 0)
    {
...
void Tetris :: equalBoards()  // I equaled my real tetris board and vector which I read from file in here...
{
    for(int i = 0; i < v1.size(); i++)
    {
        for(int j = 0; j < v1[0].size(); j++)
        {
            tetris_board[i][j] = v1[i][j];   // This is Deep Copy not Shallow copy
        }
    }
...
void Tetris :: readFromFile(const string file)
{
...
void Tetris :: writeToFile(const string file)
{
